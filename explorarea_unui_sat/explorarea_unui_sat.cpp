...
void renderCastle(const Shader& shader);
...
int main(int argc, char** argv)
{
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Exploatarea Branului", NULL, NULL);
	if (window == NULL) {
	...
	// load textures
	// -------------
	unsigned int floorTexture1 = CreateTexture(strExePath + "\\texture.jpeg");
	...
	/*	glViewport(0, 0, SHADOW_WIDTH, SHADOW_HEIGHT);
		...
		glEnable(GL_CULL_FACE);
		glCullFace(GL_FRONT);
		//renderCastle(shadowMappingDepthShader);
		glCullFace(GL_BACK);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		...
		glBindTexture(GL_TEXTURE_2D, depthMap);
		glDisable(GL_CULL_FACE);
		//renderCastle(shadowMappingShader);
